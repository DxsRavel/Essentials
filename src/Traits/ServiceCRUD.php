	...
	function agregar($new){
		...
		if($this->puedeAgregar($new)){
			...
			try{
				...
				Event::dispatch(new ModelCreatedEvent($this->LastModel, $this->Model));
				return $Model;
		...
		if($this->puedeEditar($old,$new)){			
			if( $this->LastModel = $this->existe($old) ){
				...
				try{				
					...
					Event::dispatch(new ModelUpdatedEvent($this->OldModel, $this->LastModel, $this->Model));
					return true;
				}catch(PDOException $e){			
					return false;
				}
	...
	function borrarFisico($old){
		...
		try{
			if( $this->LastModel = $this->existe($old) ){
				...
				Event::dispatch(new ModelDeletedEvent($this->OldModel, $this->Model));
				$this->LastModel = null;
				$this->title = 'Borrado Correctamente.';
				return true;
			}else{
