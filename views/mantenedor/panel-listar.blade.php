<div class="panel panel-default">
    <div class="panel-heading">
        <strong>{{ Config::has('dxsravel.maintainer-title') ? Config::get('dxsravel.maintainer-title') : 'Mantenedor' }}
            -
            {{ $Model->getName() }}</strong>
        ...
        <table class="panel-body table table" id="tblista">
            ...
            <tbody>
                @foreach ($Lista as $Item)
                    <tr>
                        @foreach ($Model->getVisible() as $column)
                            <td class="{{ $column }} {{ $Model->tdAlign($column) }}"
                                data-value="{{ $Item->$column }}">{{ $Model->normalize($Item->$column, $column) }}</td>
                        @endforeach
                        <td class="text-right">
                            @if ($puede['borrar'])
                                <a class="btn btn-xs btn-danger tooltips btn-borrar" data-placement="top"
                                    title="Borrar"><i class="fa fa-trash-o"></i></a>
                            @endif
                            @if ($puede['editar'])
                                <a class="btn btn-xs btn-warning tooltips btn-editar" data-placement="top"
